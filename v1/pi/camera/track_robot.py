...
def processKeypoints(keypoints, img, refPt):
	...
	if (len(distances) >= 2) :
		...
		#if (prob_1st_red > 0):
			#print toString(keypoints[0].pt, refPt, "R") + ", " + toString(keypoints[1].pt, refPt, "B")
		#else :
			#print toString(keypoints[1].pt, refPt, "R") + ", " + toString(keypoints[0].pt, refPt, "B")
		...
		while not self.terminated:
			...
			if self.event.wait(1):
				try:
					...
					self.pt = processImage(image[ymin:ymax, xmin:xmax], [xmin, ymin])
					if (not self.pt):
						self.pt = self.START_POINT
					...
					if ((frameCounter % 40) == 0):
						#filename = "fps_threaded_%08d.png" % frameCounter
